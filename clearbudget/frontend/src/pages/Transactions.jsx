...
function exportToCSV(transactions, formatCurrency) {
  ...
  const rows = transactions.map((tx) => [tx.date, tx.payee || '', tx.category_name || 'Uncategorized', tx.memo || '', tx.amount, tx.account_name || '', tx.cleared ? 'Yes' : 'No']);
  ...
  return (
    <div className="space-y-6 animate-fade-in">
      ...
      <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-3">
        <div>
          <h1 className="text-xl font-bold text-neutral-900 dark:text-white">Transactions</h1>
          <p className="text-sm text-neutral-500 dark:text-neutral-400 mt-0.5">{filtered.length} transaction{filtered.length !== 1 ? 's' : ''}</p>
        </div>
        <div className="flex gap-2 self-start">
          <button className="btn-secondary" onClick={() => exportToCSV(filtered, formatCurrency)}>
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-3">
          <div className="sm:col-span-2">
            <div className="relative">
              ...
              <input className="input pl-9" placeholder="Search payees, categories..." value={filter.search} onChange={(e) => setFilter({ ...filter, search: e.target.value })} aria-label="Search" />
            </div>
          </div>
          <select className="input" value={filter.account} onChange={(e) => setFilter({ ...filter, account: e.target.value })} aria-label="Filter by account">
            <option value="">All Accounts</option>
            {accounts.map((a) => <option key={a.id} value={a.id}>{a.name}</option>)}
          </select>
          <select className="input" value={filter.category} onChange={(e) => setFilter({ ...filter, category: e.target.value })} aria-label="Filter by category">
      ...
      <div className="card overflow-x-auto">
        {filtered.length === 0 ? (
          <div className="text-center py-20">
            <svg className="w-20 h-20 mx-auto text-neutral-200 dark:text-neutral-700 mb-4" viewBox="0 0 80 80" fill="none">
              <rect x="16" y="16" width="48" height="52" rx="8" stroke="currentColor" strokeWidth="2.5" />
            ...
            <h3 className="text-sm font-semibold text-neutral-900 dark:text-neutral-100">No transactions found</h3>
          ...
          <table className="table" role="table" aria-label="Transactions">
            ...
            <tbody>
              {filtered.map((tx) => (
                <tr key={tx.id}>
                  <td className="whitespace-nowrap text-xs text-neutral-500 dark:text-neutral-400 tabular-nums">{new Date(tx.date).toLocaleDateString('en-US', { month: 'short', day: 'numeric', year: 'numeric' })}</td>
          ...
          <div>
            ...
            <div className="flex gap-2">
              {['expense', 'income'].map((t) => (
                <button key={t} type="button" className={`flex-1 py-2.5 px-3 rounded-lg text-sm font-semibold transition-all ${formData.type === t ? (t === 'expense' ? 'bg-red-50 dark:bg-red-950/30 text-red-700 dark:text-red-400 ring-1 ring-red-200 dark:ring-red-800' : 'bg-emerald-50 dark:bg-emerald-950/30 text-emerald-700 dark:text-emerald-400 ring-1 ring-emerald-200 dark:ring-emerald-800') : 'bg-neutral-100 dark:bg-neutral-800 text-neutral-500 dark:text-neutral-400'}`} onClick={() => setFormData({ ...formData, type: t })}>{t === 'expense' ? '↓ Expense' : '↑ Income'}</button>
              ))}
            </div>
          </div>
          <div className="grid grid-cols-2 gap-3">
            <div><label className="block text-xs font-medium text-neutral-700 dark:text-neutral-300 mb-1.5">Account</label><select className="input" value={formData.account_id} onChange={(e) => setFormData({ ...formData, account_id: parseInt(e.target.value) })} required>{accounts.map((a) => <option key={a.id} value={a.id}>{a.name}</option>)}</select></div>
            <div><label className="block text-xs font-medium text-neutral-700 dark:text-neutral-300 mb-1.5">Category</label><select className="input" value={formData.category_id} onChange={(e) => setFormData({ ...formData, category_id: e.target.value ? parseInt(e.target.value) : '' })}><option value="">No Category</option>{categories.map((c) => <option key={c.id} value={c.id}>{c.name}</option>)}</select></div>
          </div>
          <div className="grid grid-cols-2 gap-3">
            <div><label className="block text-xs font-medium text-neutral-700 dark:text-neutral-300 mb-1.5">Date</label><input type="date" className="input" value={formData.date} onChange={(e) => setFormData({ ...formData, date: e.target.value })} required /></div>
            <div><label className="block text-xs font-medium text-neutral-700 dark:text-neutral-300 mb-1.5">Amount</label><input type="number" step="0.01" min="0" className="input" value={formData.amount} onChange={(e) => setFormData({ ...formData, amount: e.target.value })} placeholder="0.00" required /></div>
          </div>
          <div><label className="block text-xs font-medium text-neutral-700 dark:text-neutral-300 mb-1.5">Payee</label><input type="text" className="input" value={formData.payee} onChange={(e) => setFormData({ ...formData, payee: e.target.value })} placeholder="e.g., Whole Foods" /></div>
          <div><label className="block text-xs font-medium text-neutral-700 dark:text-neutral-300 mb-1.5">Memo</label><input type="text" className="input" value={formData.memo} onChange={(e) => setFormData({ ...formData, memo: e.target.value })} placeholder="Optional note" /></div>
          <div className="flex items-center gap-2"><input type="checkbox" id="tx-cleared" className="rounded border-neutral-300 dark:border-neutral-600" checked={formData.cleared} onChange={(e) => setFormData({ ...formData, cleared: e.target.checked })} /><label htmlFor="tx-cleared" className="text-xs text-neutral-700 dark:text-neutral-300">Cleared</label></div>
          <div className="flex justify-end gap-3 pt-3 border-t border-neutral-100 dark:border-neutral-800">
            <button type="button" className="btn-secondary" onClick={() => setShowModal(false)}>Cancel</button>
            <button type="submit" className="btn-primary" disabled={saving}>{saving ? 'Saving...' : editingTx ? 'Update' : 'Add'}</button>
          </div>
        </form>
      </Modal>
    </div>
